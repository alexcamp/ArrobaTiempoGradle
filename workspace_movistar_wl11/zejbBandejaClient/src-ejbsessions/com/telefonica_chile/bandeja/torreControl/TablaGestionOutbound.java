...
public class TablaGestionOutbound extends Tabla
{	
	protected transient Logger log = LoggerFactory.getLogger(getClass());

	private static SimpleDateFormat formatoFecha = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	public TablaGestionOutbound(int paginaActual) {
		super(paginaActual);		
	}

	public TablaGestionOutbound() {
		super();
	}
	...
	public List getElementosPagina(HashMap filtro) throws TablaException {
		...
		// OJO: La condicion para ATST va por el ROL.
		...
		String sql = 
			...
			" where b.act_id in ( select det.ACT_ID from atiempo.detalle_actividad_tc det where det.actc_id=7) " +
			sqlWhereFijo + " " +
...
//			Hashtable salida = db.selectReadUncommitted(sql.toString());		
	...
	public List getTitulosCuadroMando(HashMap filtro) throws TablaException {
		...
		String sql = 
				...
				"where 1=1 and  TC.actc_id=7 " + sqlWhereFijo + " " +
	...
	public List getListadoPeticionesTC(HashMap filtro) throws TablaException {
		...
		String sql = 
				...
				" and b.act_id in ( select det.ACT_ID from atiempo.detalle_actividad_tc det where det.actc_id=7) " + 
				sqlWhereFijo ;
