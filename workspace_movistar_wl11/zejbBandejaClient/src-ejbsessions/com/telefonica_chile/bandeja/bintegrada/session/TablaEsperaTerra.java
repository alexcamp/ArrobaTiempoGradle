...
import com.telefonica_chile.bandeja.torreControl.TablaNuevaTorreControl;
/**
 * @author lcaldera
 *
 */
public class TablaEsperaTerra extends Tabla
{
	private DBManager db = new DBManager();
	private static SimpleDateFormat formatoFecha = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	protected transient Logger log = LoggerFactory.getLogger(getClass());

	public TablaEsperaTerra(int paginaActual,int orden)
	{
		super(paginaActual);
...
//		//Inicio @Trace
		...
		String sql =
			"select * from ( "
				...
				+ " where me.PETI_NUMERO=b.PETI_NUMERO AND b.COD_DPT=DEPA.COD_DPT " +
					...
					"and me.COD_ESTADO=3 and me.COD_ACTIVIDAD_GENERADORA='Director de Flujos.Configuracion_Terra.Configurar_Terra' " +
	...
	private int getCantidadElementos(HashMap filtro)
	{
		...
		TraceMainConfiguration traceConf = TraceMainConfiguration.getInstance();		
		TraceManager traceManager = traceConf.getManager();		
		TraceOperation traceOp = traceManager.newOperation(TraceAdapter.getAccion(TablaNuevaTorreControl.class,"count"));
		...
		String sql = " select count(B.PETI_NUMERO) as TOTAL from vpistbba.peticion B,atiempo.mensaje_estado_ba me,vpistbba.bitacora_peticion bi" +
			" where me.COD_ESTADO=3 and me.COD_ACTIVIDAD_GENERADORA='Director de Flujos.Configuracion_Terra.Configurar_Terra'" +
