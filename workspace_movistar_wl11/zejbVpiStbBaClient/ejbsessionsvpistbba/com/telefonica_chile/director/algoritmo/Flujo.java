...
import co.com.telefonica.atiempo.ejb.eb.Flujo_definicionLocal;
import co.com.telefonica.atiempo.ejb.eb.Flujo_definicionLocalHome;
	...
	protected void addActividad(Actividad act, int pasoId) throws DirectorException {
		...
		if (pasoId >= 0 && pasoId < flujo.size()) {
			...
			for (Iterator iter = paso.iterator(); iter.hasNext();) {
				...
				if (actividad.getId().intValue()==act.getId().intValue()) {
	...
	public Actividad getActividad(Integer actividadId) {
		if (ubicacion.containsKey(actividadId)) {
			...
			for (Iterator iter = hs1.iterator(); iter.hasNext();) {
				Actividad element = (Actividad) iter.next();
				if (actividadId.intValue() == element.getId().intValue()) {
					return element;
				}
			}
		}
	...
	public void setFlujo(Integer idFlujo, TreeSet listaProductos) throws DirectorException {
		...
		try {
			...
			for (Iterator iter = actividadesPS.iterator(); iter.hasNext();) {
				...
				if (act.getId() == null || element.getActi_id().intValue()!=act.getId().intValue()) {
					if (act.getId() != null) {
						aux.add(act);
					}
					act = new Actividad();
					act.setActividad(element.getActi_id());
					try {
						...
						Actividad_flujoKey actFlujoKey = new Actividad_flujoKey( element.getActi_id() );
						...
						log.error("No encontro Actividad " + element.getActi_id() + "en la base.", e);
						throw new DirectorException("No encontro Actividad " + element.getActi_id() + "en la base.", e);
					}
	...
	private boolean esDependiente(ArrayList paso, Actividad a) {
		...
		for (Iterator iter = paso.iterator(); iter.hasNext();) {
			...
			for (Iterator iterator = dependencias.iterator(); iterator.hasNext();) {
				...
				if (actividadPasoId.intValue() == actividadDependencias.intValue()) {
					return true;
				}
			}
...
//		}
	...
	protected HashSet getActividadesParalelas(Integer pivoteId) {
		...
		for (Iterator iter = paso.iterator(); iter.hasNext();) {
			Actividad element = (Actividad) iter.next();
			if (element.getId().intValue()!= pivoteId.intValue()) {
				hs.add(element);
			}
		}
		return hs;
	...
	public boolean tieneActividad(Integer id) {
		for (int j = 0; j < flujo.size(); j++) {
			...
			for (Iterator iter = unPaso.iterator(); iter.hasNext();) {
				Actividad unaActividad = (Actividad) iter.next();
				if (id.intValue() == unaActividad.getId().intValue()) {
					return true;
				}
			}
		}
