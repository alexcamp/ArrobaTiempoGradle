...
{
	var urlnew="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/vpistbba/"+url+"&ID_USUARIO=<%=usuario.getId()%>&gestorOS=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>";	
	//alert(urlnew);
	if (newWinOS != null)
		newWinOS.close();
	var newWinOS = window.open("about:blank",'PantallaTrabajo','width=900,height=660,resizable=yes,top=0,left=0,scrollbars=yes,location=no');
	newWinOS.location.href=urlnew;
	newWinOS.focus();
}*/
function tomarFrame2(petinumero)
{
	...
	if (newWinOS != null)
		newWinOS.close();
	//alert(url);
	var newWinOS = window.open("about:blank",'PantallaBuscador','width=900,height=660,resizable=yes,top=0,left=0,scrollbars=yes,location=no');
	newWinOS.location.href=url;
...
function tomarFrame3(url)
{
	...
	if (newWinOS != null)
		newWinOS.close();
	var newWinOS = window.open("about:blank",'PantallaTrabajo','width=900,height=660,resizable=yes,top=0,left=0,scrollbars=yes,location=no');
	newWinOS.location.href=url+"&ID_USUARIO=<%=usuario.getId()%>&gestorOS=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>";
	newWinOS.focus();
}*/
// -->
</script>
     ...
     <input type="hidden" name="usuario2" value="<%=usuario.getId()%>">
		...
		<table border="0" cellpadding="0" cellspacing="0" width="780">
		    <tbody>
		        <tr>
		            <td>
		                <span class="nombre-app">@Tiempo &gt; Buscador Avanzado Gestor OS</span></td>
		            <td align="right">
...
<form name="formaBuscar" action="verBuscadorGesOS.acc" method="post">
...
<table border="0" id="filtro-grande" style="width: 800px;">
	...
	<tr class="elementos-filtro">
		...
		<td align="center">
		  <input type="text" name="rut" size=15 maxlength="20" value="<%=rut%>">-
		  <input type="text" name="rutDv" size=2 maxlength="2" value="<%=rutDv%>">
		  <br>
		</td>
		<td align="center">
			...
			%>
				<TR>
					...
					<%
						...
						{
						%>
						<a href="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/vpistbba<%=pDto.getUrlBandeja()%>&ID_USUARIO=<%=usuario.getId()%>&gestorOS=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>" target="BuscadorAvGestorOS">Ver</a>
						<%
						}
						else
						{
