...
<%-- PABLO L CR-11458 --%>
<%@page import="co.com.telefonica.atiempo.ejb.eb.Grupos_EspecialesLocal"%>
<%@page import="co.com.telefonica.atiempo.ejb.eb.CanalLocal"%>
<%-- CR23110 - PCawen - Filtro por regla --%>
<%@page import="co.com.atiempo.dto.Regla_RetencionesDTO"%>
<%@page import="javax.servlet.http.HttpSession"%>
<%@page import="com.telefonica_chile.comun.ComunInterfaces"%>
...
String    mensajeEntrada  = (String) request.getAttribute("msgEntrada");
HttpSession sesion=request.getSession();
System.out.println("verBandeja.jsp ::: mensajeEntrada = "+mensajeEntrada + " ::: sesion = "+sesion);
if(!mensajeEntrada.equals("")&&sesion.isNew()){%>
<script language="JavaScript">
 var msg = "<%=mensajeEntrada%>";
 alert('Mensajes @tiempo: '+msg);
</SCRIPT>
<%
...
//CR23110 - PCawen - Filtro por regla
ArrayList listReglas = (ArrayList) request.getAttribute("listReglas");
...
<table id="filtro-chico" style="display: none" width="100%">
	<tr class="titulo-tabla">
		...
		<td align="right" valign="bottom"><a
			...
			title="Mostrar Filtro Completo">
			<img src="/bandejaweb/img/restaurar.gif" height="14" border="0"> </a></td>
	</tr>
</table>
<form name="formaMasiva" method="post">

</form>
<form name="forma" action="verBandeja.acc" method="post">
...
<table id="filtro-grande" width="100%">
	<tr class="titulo-tabla">
		...
		<a href="javascript:oculta_filtro();" class="link-simple-filtro" title="Ocultar Filtro"> <img src="/bandejaweb/img/minimizar.gif" height="14" border="0"> </a></td>
	</tr>
...
%>
	...
	<tr class="elementos-filtro">
		...
		<!--<td colspan="3">:-->
		<td >: <INPUT type="text" name="CCN" size="5" maxlength="14" class="input-form"
	...
	<tr class="elementos-filtro">
		...
		<td>:
			...
			<select name="opcocat" id="opcocat">
				...
				<!-- Filtro para pedidos de retiros -->
				<OPTION value="B" <% if(opcocat.equals("B")){ %> selected="selected" <%}%>>Retiro</OPTION>
...
//			}
		...
		<!-- CR23110 - PCawen Filtro por Regla--> 
		<td >Regla</td>
		<TD>: <select name="regla">
			<OPTION value="">- Seleccione Regla</OPTION>			
			<%for (Iterator it = listReglas.iterator(); it.hasNext();)
			{
				Regla_RetencionesDTO regla = (Regla_RetencionesDTO) it.next();
				String valorRegla = request.getParameter("regla");%>
			<option
				<%=UtilesBandeja.selected(regla.getRegla_id().toString(), valorRegla)%>
				value="<%=regla.getRegla_id()%>"><%=regla.getDescripcion()%></option>
			<%}%>
		</select></TD>
		<!-- Fin CR23110 -->
...
for (Iterator it = peticiones.iterator(); it.hasNext(); par++) {
	...
	if(peticion.getActividadCodigo().equals(ComunInterfaces.ACT_CONTROL_DESINSTALAR_CRE)
		|| peticion.getActividadCodigo().equals(ComunInterfaces.ACT_DESINSTALAR_CRE)
		|| peticion.getActividadCodigo().equals(ComunInterfaces.ACT_CONTROL__DESINSTALAR_MTZ)
		|| peticion.getActividadCodigo().equals(ComunInterfaces.ACT_DESINSTALAR_MTZ)){
		
			url=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)+"/"+"BuscadorPeticionesCo.acc?folio="+peticion.getBiNroPeticion()+"&bandeja=RecogidaEquipos"+"&ID_USUARIO="+usuario.getId();
	}else{
	url =
...
<script>
 function accionMasiva(forma, tabla){
  ...
  //CR10394 - Pablo Cawen
  if(opcion=='cierrePorArchivo'){
  	blockButtonF();
  	jsTerminarTrabajoByFile(forma);
  }
  // - Fin -

  if (opcion=='terminarST') {
  	 blockButtonF();
	 jsTerminarTrabajoST(forma);
   }
   ...
   jsBintegrada(tabla);
	...
	if(opcion=='cancelarSTPorArchivo'){
  	blockButtonF();
  	jsCancelarTrabajoByFileST(forma);
  }
	if(opcion=='CierreRecEquip'){
  	cierresRecojidaEquip(forma);
  
  }
  if(opcion=='cierreManualVAB'){
  	jsCierreMasivoVAB(forma);
  }

   }


 function cierresRecojidaEquip(table) {
	
	document.forma.action="<%=((AplicacionDTO) mapAplicaciones.get("VPISTBBA")).getApUrlBase()%>/CierresManuales";
 	document.forma.submit();
...
function abrirVentana(url, target, ancho, alto) {
 ...
 {	
	...
	document.forma.action="<%=((AplicacionDTO) mapAplicaciones.get("VPISTBBA")).getApUrlBase()%>/GeneraArchivo?iduserAccion=<%=usuario.getId()%>&userAccion=<%=usuario.getUsername()%>";
 	document.forma.submit();
 ...
 function archivoODS(forma)
 {
 	...
 	document.forma.action="<%=((AplicacionDTO) mapAplicaciones.get("VPISTBBA")).getApUrlBase()%>/GeneraAODS?iduserAccion=<%=usuario.getId()%>&userAccion=<%=usuario.getUsername()%>";
 	document.forma.submit();
 ...
 {
 	 if(!confirm("Esta accion generará el Archivo de ODS ST Completo para el usuario. ¿Desea Continuar?\nAdvertencia: Si ya generó el archivo CANCELE y busque en la Bandeja de Reportes"))
 	{
	 	document.forma.action="verBandeja.acc";
	 	document.forma.target = "";
	 	return;
 	}
 	document.forma.action="<%=((AplicacionDTO) mapAplicaciones.get("ATST")).getApUrlBase()%>/GeneraAODS?iduserAccion=<%=usuario.getId()%>&userAccion=<%=usuario.getUsername()%>";
 	document.forma.submit();
...
/*	document.forma.action= "ProcesoBatch.acc";
 ...
 function jsTerminarTrabajoByFile(forma){
 	var ancho = 300;
	var alto = 200;
	...
	window.open('VerUploadFile.acc','Hi',propiedades);
 }
 // - Fin-
 
  function jsTerminarTrabajoST(forma){
 	document.forma.action="<%=((AplicacionDTO) mapAplicaciones.get("ATST")).getApUrlBase()%>/TerminaActividad?userAccion=<%=usuario.getUsername()%>";
 	document.forma.target="iprint";
 	document.forma.submit();
...
//  document.forma.target="iprint";
 ...
 function jsCancelarTrabajoByFileST(forma){
 	var ancho = 300;
	var alto = 200;
	hor = (screen.width - ancho)/2;
	vert = (screen.height - alto)/2;
	propiedades = eval("'toolbar=no, location=no, directories=no, status=no, menubar=no, resizable=no, scrollbars=no, width=" + ancho + ", height=" + alto + ", left=" + hor + ", top=" + vert + "'"); 
	window.open('VerUploadFileST.acc','Hi',propiedades);
 }

  function jsCierreMasivoVAB(forma){
 
   document.forma.action="CierreMasivoVAB.acc";
   document.forma.target="";
   document.forma.submit();
   document.forma.action = "verBandeja.acc";
   document.forma.target = "";
