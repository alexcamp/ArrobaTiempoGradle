...
{
	var urlnew="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/vpistbba/"+url+"&ID_USUARIO=<%=usuario.getId()%>&ginbound=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>";	
	//alert(urlnew);
	if (newWinIn != null)
		newWinIn.close();
...
function tomarFrame3(url)
{
	...
	if (newWinIn != null)
		newWinIn.close();
	var newWinIn = window.open("about:blank",'PantallaTrabajo','width=900,height=660,resizable=yes,top=0,left=0,scrollbars=yes,location=no');
	newWinIn.location.href=url+"&ID_USUARIO=<%=usuario.getId()%>&ginbound=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>";
	newWinIn.focus();
}*/
// -->
</script>
     ...
     <input type="hidden" name="usuario2" value="<%=usuario.getId()%>">
		...
		<table border="0" cellpadding="0" cellspacing="0" width="780">
		    <tbody>
		        <tr>
		            <td>
		                <span class="nombre-app">@Tiempo &gt; Buscador Avanzado Inbound</span></td>
		            <td align="right">
...
<SCRIPT>
	...
	function enviarAM()
	{
		if(!confirm("Esta accion generará el Archivo Masivo Completo . ¿Desea Continuar?\nAdvertencia: Si ya generó el archivo CANCELE y busque en la Bandeja de Reportes"))
	 	{
		 	document.formaBuscar.action="verGestionInbound.acc";
		 	document.formaBuscar.target = "";
		 	return;
	 	}
		document.formaBuscar.action="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA) %>/GeneraAODSInBound?userAccion=<%=usuario.getUsername()%>&iduserAccion=<%=usuario.getId() %>";
...
<form name="formaBuscar" action="verGestionInbound.acc" method="post">
...
<table border="0" id="filtro-grande" style="width: 800px;">
	...
	<TR class="elementos-filtro">
		<TD colspan="3" align="center">
			<input type="button" value="Buscar" class="botones-chicos" onclick="buscarIn(this.form)">
		</TD>
	</TR>
	<TR>
		<TD colspan="2" align="left">
		Generacion Archivo InBound:
			...
			%>
				<TR>
					...
					<%
						...
						{
						%>
						<a href="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/vpistbba<%=pDto.getUrlBandeja()%>&ID_USUARIO=<%=usuario.getId()%>&ginbound=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>" target="BuscadorInboundVPI">Ver</a>
						<%
						}
						else
						{
...
//				url = url +"&ID_USUARIO=" + usuario.getId() + "&ginbound=" + randomInt + "&USUARIO=" + usuario.getUsername();
//				%>
				<SCRIPT>
//					tomarFrame3('<%//=url%>');
//				</SCRIPT>
