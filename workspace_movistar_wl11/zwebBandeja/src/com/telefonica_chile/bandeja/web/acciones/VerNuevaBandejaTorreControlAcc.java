...
public class VerNuevaBandejaTorreControlAcc extends Accion{
	...
	protected void ejecutar(HttpServletRequest request) throws Evento
	{
		...
		try{
			...
			boolean tienePerfilAdministrador=false;
			try
			{
				Long idPerfil=new Long(VpistbbaConfig.getVariable("PERFIL_ADMIN"));
				tienePerfilAdministrador=biLocal.esAdministrador(usuario.getId(),idPerfil);
			}
			catch(Exception e)
			{
				log.debug("no se pudo determinar el perfil de administrador");
			}
			log.debug("tienePerfilAdministrador:"+tienePerfilAdministrador);
...
//			}
	...
	private HashMap recuperaFiltrosFijos(HttpServletRequest request)
	{
		...
		map.put("fTC_Actividad",UtilesBandeja.sinNull(request.getParameter("fTC_Actividad"),""));
