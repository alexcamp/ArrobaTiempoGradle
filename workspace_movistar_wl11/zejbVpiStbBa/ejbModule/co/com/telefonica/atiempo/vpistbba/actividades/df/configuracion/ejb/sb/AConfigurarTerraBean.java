...
/**
 * Bean implementation class for Enterprise Bean: AConfigurarTerra
 */
public class AConfigurarTerraBean
	...
	protected void onInicioActividadVPI(ActividadEJBDTO act) throws TnProcesoExcepcion {
		log.debug("--Pasa por metodo onInicioActividadVPI de AConfigurarTerraBean, en la actividad "+ act.getCodigoActividad()+", para la petición: "+act.getNumeroPeticion());
		...
		try{
			...
			Collection listaMensajeLocal =  mensajeEstadoBaLocalHome.findByPetiNumeroErrorActividad(act.getNumeroPeticion(), "6", ComunInterfaces.COD_ACT_CONFIG_TERRA_CDS);
	...
	private void limpiarErroresMensajeEstadoBA(Long petiNumero, Long correlativo){
		try{
			Mensaje_estado_baLocalHome mensajeEstadoBaLocalHome=(Mensaje_estado_baLocalHome) HomeFactory.getHome(Mensaje_estado_baLocalHome.JNDI_NAME);
			
			Collection listaMensajeLocal =  mensajeEstadoBaLocalHome.findByPetiNumero(petiNumero);
			if (!listaMensajeLocal.isEmpty()){
				for(Iterator iterator=listaMensajeLocal.iterator();iterator.hasNext();){
					Mensaje_estado_baLocal mebaLocal=(Mensaje_estado_baLocal) iterator.next();
			...
			log.error("Error al limpiarErroresMensajeEstadoBA en: " + ex);
