...
/**
 * Bean implementation class for Enterprise Bean: AConfPaqueteMovil
 */
public class AConfPaqueteMovilBean extends
	...
	protected void onInicioActividadVPI(ActividadEJBDTO act)
			throws TnProcesoExcepcion {
		log.debug("Ejecutando AConfPaqueteMovilBean: nro peticion: [ "
		...
		try {
			...
			if (!act.getDato(DATOS_ATVPISTBBA.DIRECTOR_DE_FLUJOS.fluj_reversa)
					.equals("S")) {
				recursos.configurarPaqueteMovil(act);
			} else {
			...
			throw new TnProcesoExcepcion(
					"Error en Actividad AConfPaqueteMovilBean ", appEx);
		}
	}
