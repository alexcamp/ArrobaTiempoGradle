...
/**
 * Bean implementation class for Enterprise Bean: AConfMediacionMovil
 */
public class AConfMediacionMovilBean extends
	...
	protected void onInicioActividadVPI(ActividadEJBDTO act)
			throws TnProcesoExcepcion {
		log.debug("Ejecutando AConfMediacionMovilBean: nro peticion: [ "
		...
		try {
			...
			if (!act.getDato(DATOS_ATVPISTBBA.DIRECTOR_DE_FLUJOS.fluj_reversa)
					.equals("S")) {
				recursos.configurarMediacionMovil(act);
			} else {
			...
			throw new TnProcesoExcepcion(
					"Error en Actividad AConfMediacionMovilBean ", appEx);
		}
	}
