...
/*
function tomarFrame1(url)
{
	var urlnew="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/vpistbba/"+url+"&ID_USUARIO=<%=usuario.getId()%>&goutbound=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>";	
	//alert(urlnew);
	if (newWinIn != null)
		newWinIn.close();
	var newWinIn = window.open("about:blank",'PantallaTrabajo','width=900,height=660,resizable=yes,top=0,left=0,scrollbars=yes,location=no');
	newWinIn.location.href=urlnew;
	newWinIn.focus();
}*/
function tomarFrame2(petinumero)
{
	//alert(petinumero);
	var url="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/"+"BuscadorPeticionesCo.acc?folio="+petinumero+"&bandeja=BuscadorRecogidaEquipos"+"&ID_USUARIO=<%=usuario.getId()%>";
	if (newWinIn != null)
		newWinIn.close();
	//alert(url);
...
/*
function tomarFrame3(url)
{
	//alert(url);
	if (newWinIn != null)
		newWinIn.close();
	var newWinIn = window.open("about:blank",'PantallaTrabajo','width=900,height=660,resizable=yes,top=0,left=0,scrollbars=yes,location=no');
	newWinIn.location.href=url+"&ID_USUARIO=<%=usuario.getId()%>&goutbound=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>";
	newWinIn.focus();
}*/
// -->
</script>
     ...
     <input type="hidden" name="usuario2" value="<%=usuario.getId()%>">
		...
		<table border="0" cellpadding="0" cellspacing="0" width="780">
		    <tbody>
		        <tr>
		            <td>
		                <span class="nombre-app">@Tiempo &gt; Buscador Avanzado Recogida Equipos</span></td>
		            <td align="right">
...
<SCRIPT>
	...
	function enviarAM()
	{
		if(!confirm("Esta accion generará el Archivo Masivo Completo . ¿Desea Continuar?\nAdvertencia: Si ya generó el archivo CANCELE y busque en la Bandeja de Reportes"))
	 	{
		 	document.formaBuscar.action="verGestionRecogidaEquipos.acc";
...
<form name="formaBuscar" action="verBuscadorRecogidaEquipos.acc" method="post">
...
<!-- se comenta la opcion del boton para Generacion Archivo OutBound - Ana Santos - Inicio -->	
	
	<!--<TR>
		<TD colspan="2" align="left">
		Generacion Archivo OutBound:
		<input type="button" onclick="enviarAM();" name="btnAM" value="Enviar Archivo" id="btnAM" class="botones-chicos">
		</TD>
		</TR>-->
	
	<!-- se comenta la opcion del boton para Generacion Archivo OutBound - Ana Santos - Fin -->	
			...
			%>
				<TR>
					...
					<%
						...
						{
						%>
						<a href="<%=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)%>/vpistbba<%=pDto.getUrlBandeja() %>&ID_USUARIO=<%=usuario.getId()%>&goutbound=<%= randomInt %>&USUARIO=<%=usuario.getUsername() %>" target="BuscadorOutbounVPI">Ver</a>
						<%
						}
						else
						{
...
//		else if(peticiones.size()==1)
//		{
//			PeticionDTO pDto = (PeticionDTO)peticiones.iterator().next();
//			if(pDto.isEstaEnBandeja() && pDto.isEstaEnActividadPermitida())
//			{
//				url=ApplicationConfig.getUrlBase(ApplicationConfig.APP_VPISTBBA)+"/vpistbba/"+pDto.getUrlBandeja();
				%>
				<SCRIPT>
//					tomarFrame3('<%//=url%>');
				</SCRIPT>
...
//			{				
				%>
				<SCRIPT>
//					tomarFrame2(<%//=pDto.getPetiNumero() %>);
				</SCRIPT>
				<%
//			}
//		}
