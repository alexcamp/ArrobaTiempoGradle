...
public class TimmerCierreAutVelAdiciona implements ServletContextListener {
	...
	private void iniciarProcesoAutVAB(){
		try{
			...
			//Se crean variable iniciales para la ejecuciòn del Timer CARM
			...
			if(ejecutaProceso.equals("1")){
				...
				log.debug("Timer de proceso automàtico para VAT iniciado satisfactoriamente.");
			}else{
				log.debug("El proceso automàtico para VAB se encuentra inactivo. Para activar habilite el valor ESTADO_TIMMER_VAB en 1 en la tabla ATIEMPO.PROPERTIES_BD");
			}
		} catch (NumberFormatException e) {
			log.error("Error formateando el valor de PROPERTIES_BD PERIODO_TIMER_VAB. El valor debe estar en milisegundos.");
		} catch (ParseException e) {
			log.error("Error parseando el valor de PROPERTIES_BD INICIO_TIMER_VAB, el valor debe seguir el formato yyyyMMddHHmmss. Ejemplo: 20130101235959 El primero de enero del 2013 en el último segundo del día.");
			log.error(e);
		} catch (Exception e){
			log.error(e);
		}
	...
	private void pararProcesoAutVAB(){
		try {
			String ejecutaProceso = getValorPropertiesBD("ESTADO_TIMMER_VAB");
			if(ejecutaProceso.equals("1")){
				log.debug("Deteniendo proceso automàtico para VAB...");
				getBajaVAB().stop();
				log.debug("Proceso automàtico para VAB detenido satisfactoriamente.");
			}
		} catch (Exception e) {
			log.error(e);
		}
