 ...
 */
public class VerNuevaTorreControlAcc extends Accion {
	...
	protected void ejecutar(HttpServletRequest request) throws Evento {
		try {
			...
			if ( usuario == null )
				log.error("No se encontro usuario en Session.");
			...
			if(request.getUserPrincipal()!=null)
				log.debug("Usuario EN TORRE DE CONTROL:"+request.getUserPrincipal().getName());
			
			if(usuario!=null)
				log.debug("Usuario EN TORRE DE CONTROL:"+usuario.getUsername()+" desde:"+request.getRemoteHost());
...
////			request.setAttribute("familiaPetiAtis", familia);
////			request.setAttribute("fTC_FechaCompromiso", fecha);
